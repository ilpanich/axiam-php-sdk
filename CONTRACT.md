...
| PHP | implemented — REST surface only (Laravel `AxiamMiddleware` + `Route::serveProtectedResourceMetadata()` macro; Symfony `AxiamAuthSubscriber` + `ProtectedResourceMetadataController`), CONTRACT.md §28.9's five tests on both frameworks. `declines` §28.5 rule 8's gRPC exposure and, per that same rule, offers no AMQP equivalent: this SDK's `Axiam\Sdk\Grpc` namespace is a client for AXIAM's own gRPC services (the opposite direction from §28) and its `Axiam\Sdk\Amqp` namespace is §8's HMAC-signed message bus, not a bearer-token resource-server guard — neither is the §10 guard §28 extends, so §10 (and therefore §28) stays REST-only on this SDK |
| Rust, Python, Java, Kotlin, C#, Go, Swift, C, C++ | not yet |
