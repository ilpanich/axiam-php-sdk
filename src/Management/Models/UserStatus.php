<?php

declare(strict_types=1);

// Generated by scripts/gen_management.py from management-registry.json and openapi.json.
// DO NOT EDIT -- your changes will be overwritten. Regenerate with
// `python3 scripts/gen_management.py`; CI verifies the committed output is current.

namespace Axiam\Sdk\Management\Models;

/**
 * The `UserStatus` enumeration from the server's OpenAPI document.
 */
enum UserStatus: string
{
    /** The wire value `Active`. */
    case Active = 'Active';

    /** The wire value `Inactive`. */
    case Inactive = 'Inactive';

    /** The wire value `Locked`. */
    case Locked = 'Locked';

    /** The wire value `PendingVerification`. */
    case PendingVerification = 'PendingVerification';

    /** The wire value `Anonymized`. */
    case Anonymized = 'Anonymized';

    /** The wire value `Deleted`. */
    case Deleted = 'Deleted';

    /**
     * Parses a wire value into a UserStatus.
     *
     * Throws on an unrecognised value rather than defaulting to a case: a server that has
     * learned a new state should surface as a loud error, not as whichever case was declared
     * first.
     * @throws \Axiam\Sdk\Core\AxiamException when `$value` is not a known case.
     */
    public static function fromWire(string $value): self
    {
        return self::tryFrom($value) ?? throw new \Axiam\Sdk\Core\AxiamException(
            sprintf('unknown UserStatus value "%s" — the server may be newer than this SDK', $value),
        );
    }
}
