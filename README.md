...
```php
$client = new AxiamClient('https://iam.example.com', 'organization', orgSlug: 'globex');
$result = $client->login('root@example.com', $password);
```
...
against a tenant with OPAQUE disabled. The constructor rejects a blank `$tenant` or
`$orgSlug`, whitespace included; a **null** `$orgSlug` stays fine, because that is the
organization identifier being optional rather than blank.
...
This SDK conforms to **contract 1.49**: [`CONTRACT.md`](CONTRACT.md) §1–§13 and §12.7, §14,
...
Once a client is built with `clientCert`, every request it makes presents that certificate,
so the §12 helpers **prefer the alias** over the top-level entry of the same name wherever
the document publishes one:
...
A client built without `clientCert` keeps using the top-level endpoints even when the
